...
import { SET_USER, SET_USER_BALANCE, SET_WATCHED_USER } from '../reducers/user.reducers.js'
...
export function updateUser(user) {
    return userService.update(user)
        .then(updatedUser => {
            store.dispatch({ type: SET_USER, loggedinUser: updatedUser })
            const currentWatchedUser = store.getState().userModule.watchedUser
            if (currentWatchedUser && currentWatchedUser._id === updatedUser._id) {
